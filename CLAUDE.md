...
- CMake の最新版は 4.2
- Python の最新版は 3.14
- ubuntu-slim は ubuntu-24.04 相当の軽量なイメージ
...
- Free-Threading の場合は GIL の取得と解放という表現は使わないこと
- Free-Threading の場合は nanobind の `gil_scoped_release` は「Python ランタイムからデタッチ」として使用する
- Free-Threading の場合は nanobind の `gil_scoped_acquire` は「Python ランタイムにアタッチ」として使用する
